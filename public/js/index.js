...
// Открытие/закрытие модального окна блокировки
document.querySelectorAll('#LockModalOpen').forEach(function(elem){
    elem.querySelector('img').addEventListener('click', function(){
        const modal = elem.nextElementSibling; // lockModalMenu следующий элемент
        modal.style.display = "block";
        modal.style.boxShadow = "0 0 0 max(100vh, 100vw) rgba(0, 0, 0, 0.5)";
        
        modal.querySelector('.closeLockModalmenu').addEventListener('click', function(){
...
// Обработка формы блокировки
document.querySelectorAll('.lockModalMenu').forEach(function(elem){
    elem.querySelector('#formLockModal').addEventListener('submit', function(event){
        event.preventDefault();
        fetchToggleBan(elem);
        
        window.addEventListener('click', function(e) {
            if (!e.target.closest('.lockSuccess')) {
                elem.querySelector('.lockSuccess').style.boxShadow = "none";
                elem.querySelector('.lockSuccess').style.display = "none";
                elem.style.display = "none";
                location.reload();
            }
        });
