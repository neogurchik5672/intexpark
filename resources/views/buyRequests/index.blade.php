    ...
    @foreach ($query as $item)
      <div class="card">
            ...
            <div class="dropdown-container">
                @if($item->buyRequest->where('status', '!=', 1)->count() > 0)
            <button type="button" class="btn btn_yellow dropdown-btn">
            <div>заявки</div> 
            <div><img src="{{asset('storage/products/arroy_bottom.png')}}" alt=""/></div>
          </button>
          <div class="dropdown-content">
            @foreach ($item->buyRequest->where('status', '!=', 1)->groupBy('user_id') as $buyItem)
        <a href="{{ route('buyRequest.show',$buyItem->first()->id) }}">
          {{ $buyItem->first()->user->username.' | x'.$buyItem->count()}}  
        </a>
        @endforeach
        @else
