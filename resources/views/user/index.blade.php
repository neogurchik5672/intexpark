...
<div class="scrollAllUsersUl">
    ...
    <div class="scrollbarMainAllUsersFlex">
    ...
    @foreach ($query as $item)
        <div class="item">
            ...
            <span>                
              ...
              {{strlen($item->username) > 8 ? "@".substr($item->username,0,7)."..." : "@".$item->username}}
              </div>
            </span><br>
 ...
 <span id="lockModalMenu">
    <img onmouseout="this.src='{{asset('img/lock.svg')}}'" 
         onmouseover="this.src='{{asset('img/lockHover.svg')}}'" 
         src="{{asset('img/lock.svg')}}">
</span>
            ...
            <form id="formTrashModal" data-id="{{$item->id}}" action="{{route('user.remove',$item->id)}}" method="POST">
                @csrf
                 ...
                 <div class="trashSuccess">ПОЛЬЗОВАТЕЛЬ {{strlen($item->username) > 8 ? "@".substr($item->username,0,7)."..." : "@".$item->username}} УДАЛЕН</div>
                </form>
           </div>    
                ...
                @method('PUT') <!-- Метод PUT для обновления ресурса -->
                  ...
                  <button>НАЗНАЧИТЬ</button>
                   ...
                   <div class="addAdminSuccess">
                    ПОЛЬЗОВАТЕЛЬ {{strlen($item->username) > 8 ? "@".substr($item->username,0,7)."..." : "@".$item->username}} ТЕПЕРЬ АДМИН
                   </div>
                  </form>
                  @else
